...
impl Tarball {
    ...
    /// file as it streams).
    pub fn fetch(url: &str, temp_file: &Path) -> Result<Box<dyn Archive>, ArchiveError> {
        ...
        ensure_containing_dir_exists(&temp_file)?;
        let file = File::create(temp_file)?;
        let data = Box::new(tee::TeeReader::new(response, file));
